...
def get_latency(data):
    ...
    labels = []
    for i in range(len(data)):
        if(data[i]['framework'][0] in 'HPMPC') or (data[i]['framework'][0] in 'mp-slice'):
            for function in data[i]['function'].unique():
                ...
                labels.append(data[i]['framework'][0] + " passive adversary " +str(function)) 
    ...
    for i in range(len(data)):
        if(data[i]['framework'][0] in 'HPMPC') or (data[i]['framework'][0] in 'mp-slice'):
            for function in data[i]['function'].unique():
                ...
                labels.append(data[i]['framework'][0] + " passive adversary")
        elif(data[i]['framework'][0] in 'MPyC'):
...
def get_latency_deviation(data):
    ...
    for i in range(len(data)):
        ...
        if framework == "HPMPC" or framework == "mp-slice":
            if framework == "HPMPC":
                bandwidth_filter = (data[i]['packetdrops(%)'] == 0) & (data[i]['bandwidths(Mbs)'] == 25000)
            for function in data[i]['function'].unique():
                for input_size in data[i]['input_size'].unique():
                    function_filter = data[i]['function'] == function
